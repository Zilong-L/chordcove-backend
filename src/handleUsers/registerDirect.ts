...
export async function registerUser(request: Request, env: Env): Promise<Response> {
	...
	const { results } = await env.DB.prepare(`SELECT id FROM ${USER_TABLE} WHERE email = ?`).bind(data.email).all();
	...
	await env.DB.prepare(`INSERT INTO ${USER_TABLE} (email, password) VALUES (?, ?)`).bind(data.email, hashedPassword).run();
